...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.label_type in ['boundary', 'border', 'j4', 'jcell']:
            # loss needs long tensor with shape [batch, height, width]
            label = torch.from_numpy(sample['label'])[0, :, :].to(torch.long)
            return img, label
        elif self.label_type == 'adapted_border':
            # loss needs float tensor with shape [batch, channels, height, width]
        ...
        elif self.label_type in ['dist', 'cell_dist']:
