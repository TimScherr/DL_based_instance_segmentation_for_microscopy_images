...
from segmentation.training.train_data_representations import get_label
...
def copy_train_data(source_path, target_path, idx):
    ...
    :return: None
    """
    label_names = ['img', 'dist_cell', 'dist_neighbor', 'boundary', 'border', 'adapted_border', 'j4', 'jcell', 'dist_naylor']
    for label_name in label_names:
        shutil.copyfile(str(source_path / f"{label_name}_{idx}.tif"), str(target_path / f"{label_name}_{idx}.tif"))
    return
...
def generate_data(img, mask, tra_gt, td_settings, cell_type, mode, subset, frame, path, crop_idx=0, slice_idx=None):
    ...
    cell_dist, neighbor_dist = get_label(mask=np.copy(mask), label_type='distance', max_mal=td_settings['max_mal'])
    boundary_label = get_label(mask=np.copy(mask), label_type='boundary', max_mal=td_settings['max_mal'])
    border_label = get_label(mask=np.copy(mask), label_type='border', max_mal=td_settings['max_mal'])
    adapted_border_label = get_label(mask=np.copy(mask), label_type='adapted_border', max_mal=td_settings['max_mal'])
    j4_label = get_label(mask=np.copy(mask), label_type='j4', max_mal=td_settings['max_mal'])
    jcell_label = get_label(mask=np.copy(mask), label_type='jcell', max_mal=td_settings['max_mal'])
    dist_naylor = get_label(mask=np.copy(mask), label_type='dist', max_mal=td_settings['max_mal'])

    # Adjust image dimensions for appropriate cropping
    img, mask, cell_dist, neighbor_dist, boundary_label, border_label, adapted_border_label, j4_label, jcell_label, \
    dist_naylor, tra_gt = adjust_dimensions(td_settings['crop_size'], img, mask, cell_dist, neighbor_dist,
                                            boundary_label, border_label, adapted_border_label, j4_label,
                                            jcell_label, dist_naylor, tra_gt)
    ...
    for y in range(ny):
        for x in range(nx):
            ...
            # Crop
            img_crop, mask_crop, cell_dist_crop, neighbor_dist_crop, boundary_label_crop, border_label_crop, \
            adapted_border_label_crop, j4_label_crop, jcell_label_crop, dist_naylor_crop, tra_gt_crop = \
                get_crop(x, y, td_settings['crop_size'], img, mask, cell_dist, neighbor_dist, boundary_label,
                         border_label, adapted_border_label, j4_label, jcell_label, dist_naylor, tra_gt)
            # Get crop name
            ...
            tiff.imsave(str(path / crop_quality / 'boundary_{}'.format(crop_name)), boundary_label_crop)
            tiff.imsave(str(path / crop_quality / 'border_{}'.format(crop_name)), border_label_crop)
            tiff.imsave(str(path / crop_quality / 'adapted_border_{}'.format(crop_name)), adapted_border_label_crop)
            tiff.imsave(str(path / crop_quality / 'j4_{}'.format(crop_name)), j4_label_crop)
            tiff.imsave(str(path / crop_quality / 'jcell_{}'.format(crop_name)), jcell_label_crop)
            tiff.imsave(str(path / crop_quality / 'dist_naylor_{}'.format(crop_name)), dist_naylor_crop)
...
def create_ctc_training_sets(path_data, mode, cell_type_list, split='01+02', crop_size=320, st_limit=280,
    ...
    if len(cell_type_list) > 1:
        trainset_name = hashlib.sha1(str(sorted(cell_type_list)).encode("UTF-8")).hexdigest()[:10]
